...
/* Edge 神经语音（真人级中文配音）：可选依赖，未安装或离线时自动回退 */
let edgeTTS = null;
try { edgeTTS = await import('msedge-tts'); } catch {}
const EDGE_VOICES = { '温柔女声': 'zh-CN-XiaoxiaoNeural', '阳光男声': 'zh-CN-YunxiNeural', '沉稳解说': 'zh-CN-YunyangNeural', '活泼萌音': 'zh-CN-XiaoyiNeural' };
async function edgeTtsMp3(text, voiceName) {
  const fmt = (edgeTTS.OUTPUT_FORMAT || edgeTTS.MsEdgeTTS.OUTPUT_FORMATS || {}).AUDIO_24KHZ_48KBITRATE_MONO_MP3;
...
function demoEssayHtml(scene, index, count, essay) {
  ...
  /* 原稿对照：问题类场景定位到对应原句并高亮，其余场景展示原稿节选 */
  const origAll = String((essay && essay.origText) || '').replace(/\s+/g, ' ').trim();
  const wrong = String(data.错 || '').replace(/…$/, '');
  let orig = '';
  if (data.类型 === 'issue' && wrong) {
    const at = origAll.indexOf(wrong);
    if (at >= 0) {
      const pre = origAll.slice(Math.max(0, at - 46), at), post = origAll.slice(at + wrong.length, at + wrong.length + 64);
      orig = `${at > 46 ? '…' : ''}${esc(pre)}<mark>${esc(wrong)}</mark>${esc(post)}${at + wrong.length + 64 < origAll.length ? '…' : ''}`;
    } else orig = `<mark>${esc(wrong)}</mark>`;
  } else if (origAll) {
    orig = esc(origAll.slice(0, 150)) + (origAll.length > 150 ? '…' : '');
  }
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>*{box-sizing:border-box}body{margin:0;width:1280px;height:720px;overflow:hidden;background:linear-gradient(135deg,#fdf9f0,#f7efdd 55%,#f3ead4);color:#2c2a24;font-family:Arial,'Microsoft YaHei',sans-serif}.grid{position:absolute;inset:0;background:linear-gradient(#2c2a240d 1px,transparent 1px),linear-gradient(90deg,#2c2a240d 1px,transparent 1px);background-size:72px 72px}.glow{position:absolute;inset:0;background:radial-gradient(ellipse at 80% 16%,#0f766e14,transparent 60%)}.head{position:absolute;left:78px;right:78px;top:52px;display:flex;justify-content:space-between;align-items:center;font:14px/1 monospace;letter-spacing:4px;color:#a3926b}.dots i{display:inline-block;width:9px;height:9px;border-radius:50%;background:#2c2a2420;margin-left:9px}.dots i.on{background:#0f766e}.card{position:absolute;left:78px;right:78px;top:118px;bottom:196px;background:#fffdf7;border:1px solid #eadfca;border-radius:26px;padding:42px 56px;display:flex;flex-direction:column;justify-content:center;box-shadow:0 18px 50px rgba(120,95,40,.10);animation:rise .9s both}.tag{align-self:flex-start;display:flex;align-items:center;gap:14px;font-size:17px;letter-spacing:2px;color:#fff;background:#0f766e;padding:9px 22px;border-radius:999px;margin-bottom:28px;animation:rise .9s .1s both}.tag span{background:#0b4f4a;color:#99e2d4;font-size:14px;padding:3px 12px;border-radius:999px}.score{display:flex;align-items:baseline;gap:34px}.num{font-size:126px;line-height:1;font-weight:600;color:#b45309}.num small{font-size:28px;margin-left:8px;color:#a3926b}.badge{font-size:24px;letter-spacing:2px;color:#713f12;background:#fde68a;padding:8px 22px;border-radius:999px}.meta{font-size:22px;color:#7a6d54;margin:24px 0 30px}.bars{display:grid;grid-template-columns:1fr 1fr;gap:18px 46px}.bar{display:flex;align-items:center;gap:14px;font-size:18px;color:#57503f}.bar span{width:56px;flex:none}.track{flex:1;height:10px;border-radius:999px;background:#eee3cd;overflow:hidden}.track i{display:block;height:100%;border-radius:999px;background:#0f766e}.pair{display:flex;flex-direction:column;gap:14px}.wrong{font-size:24px;font-family:Georgia,'Times New Roman',serif;background:#fee2e2;border:1px solid #fecaca;color:#b91c1c;border-radius:14px;padding:14px 22px}.arrow{color:#a16207;font-size:20px;text-align:center}.right{font-size:24px;font-family:Georgia,'Times New Roman',serif;background:#dcfce7;border:1px solid #bbf7d0;color:#15803d;border-radius:14px;padding:14px 22px}.tip{font-size:19px;color:#7a6d54;margin-top:22px}.swap{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:13px}.swap li{display:flex;align-items:center;gap:16px;font-size:21px}.swap .old{font-family:Georgia,'Times New Roman',serif;background:#fee2e2;color:#b91c1c;padding:6px 16px;border-radius:10px;text-decoration:line-through}.swap b{color:#a16207}.swap .new{font-family:Georgia,'Times New Roman',serif;background:#dcfce7;color:#15803d;padding:6px 16px;border-radius:10px}.list{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:15px}.list li{font-size:21px;color:#4a4436;padding-left:34px;position:relative}.list li::before{content:'';position:absolute;left:4px;top:11px;width:11px;height:11px;border-radius:50%;background:#0f766e}.orig{position:absolute;left:78px;right:78px;bottom:26px;background:#fff8e4;border:1px solid #f0dfae;border-left:6px solid #d9a441;border-radius:14px;padding:13px 22px;animation:rise .9s .2s both}.olabel{display:block;font-size:13px;letter-spacing:2px;color:#a16207;margin-bottom:6px}.orig p{margin:0;font-size:17px;line-height:1.7;color:#57503f;font-family:Georgia,'Times New Roman',serif;font-style:italic}.orig mark{background:#ffe9a8;color:#8a5a00;padding:2px 6px;border-radius:6px;font-weight:600;font-style:normal}@keyframes rise{from{opacity:0;transform:translateY(26px)}to{opacity:1;transform:none}}</style></head><body><div class="grid"></div><div class="glow"></div><div class="head"><span>ESSAY REVIEW</span><span class="dots">${dots}</span></div><div class="card">${body}</div>${orig ? `<div class="orig"><span class="olabel">${data.类型 === 'issue' ? '写作原稿 · 讲解对应句' : '写作原稿 · 节选'}</span><p>${orig}</p></div>` : ''}</body></html>`;
  ...
  if (stage === 'visuals') return { visuals: project.scenes.map((scene, i) => ({ kind: 'html', html: demoEssayHtml(scene, i, project.scenes.length, essay), source: '作文讲解模板' })) };
...
export function createProvider(assetRoot, config = {}) {
  ...
  async function generate(stage, project) {
    ...
    if (stage === 'audio') {
      ...
      const audio=[];
      for (let i=0;i<project.scenes.length;i++) {
        let b, ext='wav', seconds=0, audioSource='';
        if(remote) { requireValue(typeof remote.audio[i].base64==='string'&&remote.audio[i].base64.length<12*1024*1024,'WAV 声音数据无效',502); b=Buffer.from(remote.audio[i].base64,'base64'); }
          ...
          // 首选 Edge 神经语音（真人级中文配音，Windows/Linux 皆可，需联网）；
          // 失败时 Windows 回退系统 SAPI；仍不行则生成等长静音音轨保证流程完整。
          if (edgeTTS) {
            const isEn = String(project.language||'').toLowerCase().startsWith('en');
            const voiceKey = !isEn && essay && essay.voice && EDGE_VOICES[essay.voice] ? essay.voice : '温柔女声';
            try {
              b = await edgeTtsMp3(project.scenes[i].narration, isEn ? 'en-US-AriaNeural' : EDGE_VOICES[voiceKey]);
              ext = 'mp3';
              seconds = Math.max(1, b.length * 8 / 48000); // 48 kbps 单声道 MP3
              audioSource = isEn ? 'Edge 神经语音 · 英文' : 'Edge 神经语音 · ' + voiceKey;
            } catch { b = null; }
          }
          if (!b && process.platform==='win32') {
            ...
            try { await exec('powershell.exe',['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',join(here,'speech.ps1'),'-InputPath',input,'-OutputPath',output],{windowsHide:true,timeout:60000}); b=await readFile(output); seconds=wavDuration(b); audioSource='Windows SAPI'; }
            catch { b=null; }
            finally { await unlink(input).catch(()=>{}); await unlink(output).catch(()=>{}); }
          }
          if (!b) {
            // 无任何语音引擎（离线 Linux 等）：生成与分镜等长的静音音轨，
            // 让「配音 → 预览 → 导出」整条流程仍能完整跑通。
            seconds=Math.max(1,Math.min(60,(Number(project.scenes[i].duration)||8)-0.5));
            b=silentWav(seconds); audioSource='silent';
          }
        }
        if(!seconds)seconds=wavDuration(b);
        audio.push({url:await asset(b,ext),seconds,source:remote?'gateway':audioSource});
      }
