...
## 新增：AI 英语作文批改（WriteRight）

启动网站后打开 **http://127.0.0.1:4173/essay**，或从首页右上角「✦ AI 作文批改」进入。

面向大学英语写作教学：学生登录后提交英文作文，系统给出综合得分与 CEFR 等级估算、逐句修改留痕（红色删除 + 绿色建议，鼠标悬停显示中文讲解）、按类别汇总的问题清单、用词与句式提升建议，以及「原文 / 修改后」对照，并支持导出批改报告。教师账号可查看班级全部提交与成绩分布。

- 演示账号：学生 `stu01 / 123456`、教师 `teacher / 123456`，也可自行注册。
- 语言诊断默认使用**本地规则引擎**（内置 40 余条规则：拼写、冠词、主谓一致、时态、非谓语动词、名词单复数、固定搭配、中式英语、标点、大小写、用词升级、篇章连贯等），不联网即可运行。
- 可在页面「系统设置」中填入任意兼容 OpenAI 协议的接口（DeepSeek / OpenAI / 通义 / 智谱等），启用大模型增强批改；密钥仅保存在浏览器本地，不上传服务器。
- 账号、作文与批改记录保存在浏览器 localStorage，属单机演示版本；多人在线需接入后端账号与存储服务。
- 页面采用独立浅色主题，与深色制作后台互不影响；该文件也可单独双击打开使用。

### 讲解视频：把批改结果直接变成讲解片

在批改结果的「总览」页点 **🎬 生成讲解视频**，视频 Agent 自动完成五步：

1. **撰写讲解脚本** —— 依据真实批改数据（得分、CEFR、亮点、高频问题、用词建议）生成六幕以内的讲解稿；
2. **规划讲解章节** —— 总览 → 亮点 → 主要问题（最多两类，含原句与修改对照）→ 用词升级 → 练习建议；
3. **拆分讲解分镜** —— 每幕含画面要点、旁白与时长；
4. **绘制讲解画面** —— 本地模板渲染 1280×720 讲解卡（评分卡 / 问题对照卡 / 替换表 / 要点列表），纯 CSS 动画；
5. **合成中文配音** —— Windows SAPI 真实合成 WAV，成片时长自动适配配音长度。

完成后点「打开视频工作台 →」跳到制作台（`/?project=<id>`），可逐幕预览、修改旁白与字幕，再用「↥ 录屏输出」导出成片。

实现要点：批改结果以 `template:'essay'` 随项目创建一并提交（`POST /api/projects` 的 `essay` 字段），生成引擎在 `lib/provider.mjs` 的「作文讲解模板」中按数据渲染画面与旁白。**此路径固定使用本地生成**，保证画面与批改数据严格对应，即使配置了 AI 网关也不会被改写。

## 新增：小小配音家（学生练习端）

启动网站后打开 **http://127.0.0.1:4173/english**。原有制作后台仍在 `/`。

第一课“动物朋友”包含 3 句英语。使用顺序：开始配音 → 看动画 → 听英语示范 → 点击“我来配音”并允许麦克风 → 倒数后说英语 → 停止 → 回放或保存配音视频。

- 示范使用本机 **英语**语音生成并缓存，正常播放与慢速播放可选。
- 录音会与动画一起保存为本地视频，支持按句回放、重录和下载。
- 学生录音仅存于当前页面内存，不上传后台；关闭或刷新前需要下载保存。
- 当前进度表示录制完成，不代表发音正确；AI 发音识别和针对性反馈尚未接入。
- 目前只有一个练习课程。下一步在这条学习流程验证后，再增加课程和 AI 反馈。
...
frameflow/
  ...
  public/essay.html   AI 英语作文批改模块（独立单页应用）
  public/english.html 小小配音家学生练习端
